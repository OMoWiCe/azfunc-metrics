...
def estimate_live_count(wifi_count, cellular_count, avg_devices_per_person, avg_sims_per_person, wifi_usage_ratio, cellular_usage_ratio):
    ...
    estimated_occupancy = int(round(estimated_people_from_wifi + estimated_people_from_cell, 0))
    logging.info(f"Estimated people from Wi-Fi: {estimated_people_from_wifi}, Estimated people from Cellular: {estimated_people_from_cell}, Estimated Count: {estimated_occupancy}")
...
def process_metrics(azeventhub: func.EventHubEvent):
    ...
    for message in azeventhub:
        ...
        if datetime.strptime(local_timestamp, '%Y-%m-%dT%H:%M:%S.%fZ').minute % (update_interval*5) == 0:
            turnover_time = calculate_turnover_time(location_id)

        # Step 6: Estimate live count
        live_count = estimate_live_count(len(wifi_occupancy_list), len(cellular_occupancy_list), avg_devices_per_person, avg_sims_per_person, wifi_usage_ratio, cellular_usage_ratio)

        # Step 7: Update the MAIN_METRICS table
        try:
            connection = pyodbc.connect(SQL_CONNECTION_STRING)
            cursor = connection.cursor()
            local_timestamp_formatted = datetime.strptime(local_timestamp, '%Y-%m-%d %H:%M')

            if turnover_time != 0:
                cursor.execute("""
                    IF EXISTS (SELECT 1 FROM MAIN_METRICS WHERE LOCATION_ID = ? AND [DATE] = CAST(? AS DATE))
                    BEGIN
                        UPDATE MAIN_METRICS
                        SET LIVE_COUNT = ?,
                            TURNOVER_TIME = ?
                            CREATED_AT = ?
                        WHERE LOCATION_ID = ? AND [DATE] = CAST(?) AS DATE);
                    END
                    ELSE
                    BEGIN
                        ...
                        VALUES (?, CAST(? AS DATE), ?, ?, ?);
                    END
                """, (location_id, local_timestamp_formatted, live_count, turnover_time, local_timestamp, location_id, local_timestamp_formatted, location_id, local_timestamp_formatted, live_count, turnover_time, local_timestamp))
            else:
                ...
                cursor.execute("""
                    ...
                    """, location_id)
                last_turnover_time = cursor.fetchone()
                ...
                cursor.execute("""
                    ...
                    BEGIN
                        ...
                        VALUES (?, CAST(? AS DATE), ?, ?, ?);
                    END
                """, (location_id, local_timestamp_formatted, live_count, local_timestamp, location_id, local_timestamp_formatted, location_id, local_timestamp_formatted, live_count, last_turnover_time, local_timestamp))
